"""
main.py — PowerShell Radio Pro — Entry Point
Python-based internet radio for Windows PowerShell.
Uses python-vlc (libvlc) — no WSL2, no cava, no subprocess hacks.

Usage:
  python -m radio_ps                        # start with Top Charts
  python -m radio_ps -c jazz            # start on Jazz category
  python -m radio_ps -c hindi          # start on Hindi
  python -m radio_ps --check           # verify VLC + dependencies
"""
...
Examples:
  ...
  python -m radio_ps --check
    ...
    try:
        ...
        print(f"\nImport error: {e}")
        print("Run:  python -m radio_ps --check  to diagnose\n")
        sys.exit(1)
